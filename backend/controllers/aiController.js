...
export const suggestHabit = async (req, res) => {
  try {
...
export const recoveryPlan = async (req, res) => {
  try {
    ...
    const userMsg = `Habit: ${habit.name} (${habit.category}).\nDescription: ${habit.description || "none"}.\nCurrent streak: ${current} days. Longest over: ${longest} days. The user just broke a streak. Write a warm, actionable 3-day recovery plan.`;
    const { content } = await chatCompletion({
...
export const chatAnalysis = async (req, res) => {
  try {
    ...
    const context = habits.map((h) => {
      ...
      return `${h.name} (${h.category}): ${hlogs.length}/30 in last 30 days, by weekday [Sun,Mon,Tue,Wed,Thr,Fri,Sat]: ${byDow.join(",")}`;
    });
...
export const morningMotivation = async (req, res) => {
  try {
    ...
    const userMsg = `Today's habits and streaks:\n${ctx}\n\nDone today: ${done}/${total}. Write the morning message for this user now.`;
    const { content } = await chatCompletion({
