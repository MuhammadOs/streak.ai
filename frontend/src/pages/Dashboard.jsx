...
import { DashboardSkeleton } from "../components/SkeletonCard.jsx";
...
export default function Dashboard() {
  ...
  const toggle = async (habit) => {
    ...
    if (done) {
      ...
      // Remove from weekLogs so WeeklyGrid updates
      setWeekLogs((logs) =>
        logs.filter(
          (l) =>
            !(
      ...
      // Remove from allLogsByHabit so streaks update
      ...
      setHeatmap((prev) =>
        prev.map((d) =>
          d.date === today ? { ...d, count: Math.max(0, d.count - 1) } : d,
        ),
      );
    } else {
      const res = await api.post("/logs", { habitId: habit._id, date: today });
      setTodayLogs((logs) => [...logs, res.data]);
      // Add to weekLogs so WeeklyGrid updates
      setWeekLogs((logs) => [...logs, res.data]);
      // Add to allLogsByHabit so streaks update
      setAllLogsByHabit((prev) => {
        const next = { ...prev };
      ...
      setTimeout(() => {
        ...
        if (nextDone === habits.length && habits.length > 0) {
          celebrateBig();
          toast("All habits done for today! 🎉", "success", 5000);
        }
      }, 150);
    }
  };
  ...
  const archiveHabit = async (habit) => {
    ...
    if (res.data.isArchived) {
      setHabits((hs) => hs.filter((h) => h._id !== habit._id));
      toast(`"${habit.name}" archived`, "info");
    } else {
  ...
  // Split habits into pending and done for sorted display
  const pendingHabits = habits.filter(
    (h) => !completedToday.has(String(h._id)),
  );
  const doneHabits = habits.filter((h) => completedToday.has(String(h._id)));
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card p-5">
        ...
        {habits.length === 0 ? (
          /* ── Enhanced empty / onboarding state ── */
          <div className="py-10 text-center space-y-5">
            ...
            <div>
              ...
              <div className="text-sm text-muted mt-1 max-w-xs mx-auto">
            ...
            <div className="inline-flex flex-col gap-2 text-left mx-auto">
              {[
                { n: 1, text: "Create a habit" },
                { n: 2, text: "Check it off today" },
                { n: 3, text: "Watch your streak grow" },
              ].map(({ n, text }) => (
                <div
                  key={n}
                  className="flex items-center gap-3 text-sm text-soft"
                >
                  <span className="w-6 h-6 rounded-full bg-brand-500/15 text-brand-600 dark:text-brand-300 text-xs font-bold flex items-center justify-center shrink-0">
                    {n}
                  </span>
                  {text}
                </div>
            ...
            <div className="flex items-center justify-center gap-3 pt-1">
              ...
              <button
                ...
                onClick={() => setSuggestOpen(true)}
              >
                <Sparkles size={14} /> Let AI suggest one
              </button>
            </div>
          </div>
        ) : (
